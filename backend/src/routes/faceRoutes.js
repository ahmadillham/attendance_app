...
router.post('/register', authMiddleware, upload.single('faceImage'), async (req, res) => {
    try {
        const studentId = req.user.id;

        if (!req.file) {
            return res.status(400).json({ message: 'Foto wajah wajib dilampirkan.' });
        }

        if (!isAvailable()) {
            return res.status(503).json({ 
                message: 'Layanan face recognition tidak tersedia. Model ML belum dimuat di server.' 
            });
        }
        ...
        if (!descriptor) {
            return res.status(400).json({ 
                message: 'Wajah tidak terdeteksi dalam foto. Pastikan:\n- Wajah menghadap kamera\n- Pencahayaan cukup\n- Hanya satu wajah dalam frame' 
            });
        }
