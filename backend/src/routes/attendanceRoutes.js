...
const {
  extractDescriptor,
  extractLandmarks,
  isAvailable: isFaceModelAvailable,
} = require("../lib/faceDescriptor");
    ...
    try {
      ...
      const validationData = {
        ...
        meetingCount: req.body.meetingCount
          ? parseInt(req.body.meetingCount)
          : undefined,
      ...
      const clientNow = req.body.clientTime
        ? new Date(req.body.clientTime)
        : new Date();
      ...
      if (
        diffMinutes < -EARLY_OPEN_MINUTES ||
        diffMinutes > LATE_CLOSE_MINUTES
      ) {
      ...
      // Auto-calculate meeting count based on week number from first meeting
      if (!meetingCount) {
        ...
        if (firstAttendance) {
          // Calculate which week number this is based on the first meeting date
          const firstDate = new Date(firstAttendance.date);
          firstDate.setHours(0, 0, 0, 0);
          const currentDate = new Date(clientNow);
          currentDate.setHours(0, 0, 0, 0);

          const msDiff = currentDate.getTime() - firstDate.getTime();
          const weeksDiff = Math.round(msDiff / (7 * 24 * 60 * 60 * 1000));
          meetingCount = weeksDiff + 1; // Week 0 = meeting 1
        } else {
          meetingCount = 1;
        }
      }
      ...
      if (!uploadedFiles.length) {
        return res.status(400).json({
          message:
            "Akses ditolak: Foto wajah (FaceID) wajib dilampirkan untuk verifikasi.",
        });
      }
      ...
      if (!student || !student.faceDescriptor) {
        return res.status(400).json({
          message:
            "Akses ditolak: Data biometrik wajah belum terdaftar di sistem.",
        });
      }
      ...
      if (isFaceModelAvailable()) {
        ...
        if (uploadedFiles.length >= 2) {
          ...
          const lastLandmarks = await extractLandmarks(
          ...
          for (let i = 1; i < uploadedFiles.length; i++) {
            const frameDescriptor = await extractDescriptor(
          ...
          faceVerified = true;
          console.warn(
            `⚠️  Attendance from student ${studentId}: identity verified but liveness NOT checked (single frame).`,
          );
        }
      } else {
        ...
        if (process.env.NODE_ENV === "production") {
          return res.status(503).json({
            message:
              "Layanan verifikasi wajah sedang tidak tersedia. Silakan coba lagi nanti atau hubungi administrator.",
          });
        }
        faceVerified = false;
        console.warn(
          `⚠️  Face verification UNAVAILABLE — attendance will be flagged as unverified (faceVerified=false) for student ${studentId}`,
        );
      }
      ...
      if (distance > ALLOWED_RADIUS) {
        return res
          .status(403)
          .json({ message: "Anda berada di luar area kampus." });
      }

      // Security 3: Check for duplicate attendance on the same (client) day
      const startOfClientDay = new Date(clientNow);
      startOfClientDay.setHours(0, 0, 0, 0);
      const endOfClientDay = new Date(clientNow);
      ...
      if (existingToday) {
        return res.status(409).json({
          message: `Anda sudah absen untuk mata kuliah ini hari ini.`,
        });
      }
      ...
      try {
        ...
        if (createErr.code === "P2002") {
          return res.status(409).json({
            message: `Konflik: Absensi pertemuan ke-${meetingCount} untuk mata kuliah ini sudah tercatat (mencegah duplikasi simultan).`,
          });
        }
        throw createErr;
      }
...
router.get("/history", authMiddleware, studentMiddleware, async (req, res) => {
  try {
    ...
    for (const cid of Object.keys(courseData)) {
      ...
      const allCoveredMeetings = new Set([
        ...cd.presentMeetings,
        ...cd.leaveMeetings,
      ]);

      // Calculate expected meetings based on the first recorded meeting and current client date
      const firstRecord = cd.records.find((r) => r.meetingCount === 1);
      if (firstRecord) {
        const baseDate = new Date(firstRecord.date);
        baseDate.setHours(0, 0, 0, 0);

        const now = req.query.date ? new Date(req.query.date) : new Date();
        now.setHours(23, 59, 59, 999);

        const msDiff = now.getTime() - baseDate.getTime();
        const expectedMeetings =
          Math.floor(msDiff / (7 * 24 * 60 * 60 * 1000)) + 1;

        if (expectedMeetings > cd.maxMeeting) {
          cd.maxMeeting = Math.min(expectedMeetings, 14); // cap to 14 meetings
        }
      }
      ...
      for (let m = 1; m <= cd.maxMeeting; m++) {
        if (!allCoveredMeetings.has(m)) {
          ...
          const baseDate2 = firstRecord
            ? new Date(firstRecord.date)
            : new Date();
          const estimatedDate = new Date(
            baseDate2.getTime() + (m - 1) * 7 * 24 * 60 * 60 * 1000,
          );
