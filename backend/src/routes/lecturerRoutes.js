  ...
  try {
    ...
    if (!["present", "absent", "leave"].includes(status)) {
      return res
        .status(400)
        .json({
          message: "Status tidak valid. Gunakan: present, absent, leave",
        });
    }
...
router.put("/leave-requests/:id", async (req, res) => {
  try {
    ...
    if (!existing)
      return res
        .status(404)
        .json({ message: "Pengajuan izin tidak ditemukan" });

    if (existing.course.lecturerId !== req.user.id) {
      return res
        .status(403)
        .json({
          message: "Akses ditolak: Anda bukan pengampu mata kuliah ini.",
        });
    }
