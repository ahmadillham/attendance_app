...
class _AttendanceScreenState extends State<AttendanceScreen> with TickerProviderStateMixin {
  ...
  void _handleCapture() {
    ...
    //    2. POST photo to face-recognition API
    //    3. POST attendance record with coords + timestamp

    Future.delayed(const Duration(milliseconds: 2500), () {
      if (!mounted) return;
      final faceRecognized = Random().nextDouble() < 0.8;
      _scanAnimController.stop();
      setState(() {
        _isScanning = false;
        if (faceRecognized && _isInRange) {
          _result = {'success': true, 'message': 'Absensi berhasil!\nWajah terverifikasi dan lokasi valid.'};
        } else if (!faceRecognized) {
          _result = {'success': false, 'message': 'Wajah tidak dikenali.\nSilakan coba lagi atau hubungi admin.'};
        } else {
          _result = {
            'success': false,
            'message': 'Lokasi di luar jangkauan kampus\n(${_distance?.round()}m dari kampus, maks ${Campus.allowedRadiusMeters.round()}m)'
          };
        }
        _showResult = true;
      });
